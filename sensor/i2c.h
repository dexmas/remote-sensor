...
// bit). 
void __attribute__ ((noinline)) i2c_start(uint8_t addr) __attribute__ ((used)); 
...
void __attribute__ ((noinline)) i2c_write(uint8_t value) asm("ass_i2c_write") __attribute__ ((used));

// Read one byte. If <last> is true, we send a NAK after having received 
// the byte in order to terminate the read sequence. 
uint8_t __attribute__ ((noinline)) i2c_read(byte last) __attribute__ ((used));
...
#define SDA_DDR     (_SFR_IO_ADDR(PORTB) - 1)
#define SCL_DDR     (_SFR_IO_ADDR(PORTB) - 1)
#define SDA_OUT      _SFR_IO_ADDR(PORTB)
#define SCL_OUT      _SFR_IO_ADDR(PORTB)
#define SDA_IN		(_SFR_IO_ADDR(PORTB) - 2)
#define SCL_IN		(_SFR_IO_ADDR(PORTB) - 2)
